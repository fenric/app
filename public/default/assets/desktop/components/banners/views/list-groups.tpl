	...
	<nav class="top">
		<div class="clearfix">
			<div class="btn-group pull-left">
				<button type="button" class="btn btn-sm btn-success" onclick="$desktop.component('banners.groups').add()">
				...
				{{repeat items.items}}
					<tr data-id="{{id}}">
						...
						<td>
							...
							<br><span>{#banner:{{code}}#}</span></p>

							<p><strong>PHP код для вставки в шаблон</strong>
							<br><span>&lt;?= fenric('banner::{{code}}') ?&gt;</span></p>
						</td>
						<td>
						...
						<td>
							<div class="btn-group-vertical btn-block">
								<button class="btn btn-block btn-sm btn-warning" type="button" onclick="$desktop.component('banners.groups').edit({{id}})">
...
{{when items.pagination.have is true}}
	<ul class="pagination">
		{{when items.pagination.links.current is greater than | this.items.pagination.links.first}}
			<li>
				<a href="javascript:$desktop.component('banners.groups').list({page: {{items.pagination.links.first}}})">
					<i class="fa fa-angle-double-left" aria-hidden="true"></i>
				</a>
			</li>
			<li>
				<a href="javascript:$desktop.component('banners.groups').list({page: {{items.pagination.links.previous}}})">
		...
		{{list pages start=items.pagination.links.start end=items.pagination.links.end as=page}}
			...
			{{when page is not equal | this.__parent__.items.pagination.links.current}}
				<li>
					<a href="javascript:$desktop.component('banners.groups').list({page: {{page}}})">
						<span>{{page}}</span>
					</a>
				</li>
			{{endwhen page}}
		...
		{{when items.pagination.links.current is less than | this.items.pagination.links.last}}
			<li>
				<a href="javascript:$desktop.component('banners.groups').list({page: {{items.pagination.links.next}}})">
					<i class="fa fa-angle-right" aria-hidden="true"></i>
				</a>
			</li>
			<li>
				<a href="javascript:$desktop.component('banners.groups').list({page: {{items.pagination.links.last}}})">
